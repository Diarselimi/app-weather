...
var appid = "&appid=8ec04d9381e406d72151eb4d84b33d5b";
var celsius = "&units=metric";

var isCelsius = false;
var unitIcon = '<i class="wi wi-fahrenheit"></i>';
...
function sunTravel(start, current, end) {
    ...
    current = Math.round((current / total) * 97);
...
function updateData(data) {
    ...
    $('.card-title').html(data.name + ' ,' + data.sys.country + '  ' + data.main.temp + unitIcon);
    var weather = data.weather[0];
    $('.description').html(weather.main + ' ' + weather.description);
    $('.humidity').html(data.main.humidity + ' <i class="wi wi-humidity"></i>');
    $('.min').html(data.main.temp_min + unitIcon);
    $('.max').html(data.main.temp_max + unitIcon);
    $('.timeis').html(moment.unix(data.dt).hour() + ':' + moment.unix(data.dt).minute());
